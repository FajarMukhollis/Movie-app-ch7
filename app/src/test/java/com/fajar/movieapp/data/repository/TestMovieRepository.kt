...
import com.fajar.movieapp.MainDispatcherRule
import com.fajar.movieapp.data.network.datasource.MovieRemoteDataSource
import com.fajar.movieapp.data.network.model.HomeMovieItem
...
class TestMovieRepository {
    ...
    fun setUp() {
        apiService = FakeApi()
        dataSource = FakeRemoteDataSource(apiService)
    ...
    fun `when popular movie should exist in popular movie list`() = runTest {
        ...
        Assert.assertTrue(samplePopularMovie.contains(sample))
        Assert.assertTrue(samplePopularMovie.contains(sample2))
    }
